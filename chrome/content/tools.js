...
tools.fixsubject = function () {
  ...
  for (const item of zitems) {
    ...
    Zotero.HTTP.doGet('http://api.uread.today/master/anon/ch_lib_cls/info?code=' + clc, function (request) {
          ...
          if (json && json.resultcode === 1) {
            ...
            if (json.data.subject) {
              ...
              if (!collection) {
                ...
                let collections = zitem.getCollections()
                ...
                await zitem.saveTx()
                pw.addLines(`${zitem.getField('title')}`, `chrome://zotero/skin/tick${Zotero.hiDPISuffix}.png`)
              }
            } else {
