...
if (!Zotero.ZotURead.Notes) Zotero.ZotURead.Notes = {};

Zotero.ZotURead.Notes = Object.assign(Zotero.ZotURead.Notes, {
	init() {
		Zotero.ZotURead.Logger.log('Zotero.ZotURead.Notes inited.');
  ...
  statisticsToText(statistics) {
    ...
    let word_params = {
      ...
      cn_scale: statistics.length > 0 ? (Zotero.ZotURead.Utils.scale(statistics.cn_words / statistics.length, 1) + '%') : '-',
      en_words: statistics.en_words.toString(),
      en_scale: statistics.words > 0 ? (Zotero.ZotURead.Utils.scale(statistics.en_words / statistics.length, 1) + '%') : '-',
      num_words: statistics.num_words.toString(),
      num_scale: statistics.words > 0 ? (Zotero.ZotURead.Utils.scale(statistics.num_words / statistics.length, 1) + '%') : '-',
      other_words: statistics.words - statistics.cn_words - statistics.en_words - statistics.num_words,
      other_scale: statistics.words > 0 ? (Zotero.ZotURead.Utils.scale((statistics.words - statistics.cn_words - statistics.en_words - statistics.num_words) / statistics.length, 1) + '%') : '-',
    };
    ...
    let displayStore = Zotero.ZotURead.Utils.displayStore(statistics.sizes);
    return {
      text: word,
      title: Zotero.ZotURead.L10ns.getString('zoturead-words-title', word_params),
      space: `${displayStore.text} (${Zotero.ZotURead.L10ns.getString('zoturead-sizes-contentscale', word_params)}${statistics.sizes > 0 ? Zotero.ZotURead.Utils.scale(statistics.words / statistics.sizes, 1) : 0}%)`
    };
  },
  ...
  search: function (libraryID, collectionKey, savedSearchKey, excludeCollectionKeys) {
    Zotero.ZotURead.Logger.log({libraryID, collectionKey, savedSearchKey, excludeCollectionKeys});
    ...
    if (Zotero.ZotURead.Objects.isNoEmptyArray(excludeCollectionKeys)) {
  ...
  links(noteID) {
    ...
    if (note.parentItem) {
      links.push(...Zotero.ZotURead.Items.links(note.parentItemID));
    } else {
        let collectionID = note.getCollections()[0];
        links.push(...Zotero.ZotURead.Collections.links(collectionID));
    }
